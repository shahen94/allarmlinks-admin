...
const Volunteers = () => {
    ...
    const CellClickHandler = (id: string, e: React.MouseEvent): void => {
        if (!e.ctrlKey) history.push(`/volunteers/${id}`);
    };
                    ...
                    {fetchVolunteersStatus === ActionStatus.Success && <Table size="small" aria-label="Volunteers">
                        ...
                        <TableBody>
                            {volunteers && volunteers.map((row, index) => {
                                return (
                                    <TableRow key={row._id} className="tr-clickable">
                                        <TableCell align="left"
                                            onClick={(e) => CellClickHandler(row._id, e)}>{row.name}</TableCell>
                                        <TableCell align="left"
                                            onClick={(e) => CellClickHandler(row._id, e)}>{row.surname}</TableCell>
                                        <TableCell align="left"
                                            onClick={(e) => CellClickHandler(row._id, e)}>{row.email}</TableCell>
                                        <TableCell align="left"
                                            onClick={(e) => CellClickHandler(row._id, e)}>{row.phone}</TableCell>
                                        <TableCell align="left"
                                            onClick={(e) => CellClickHandler(row._id, e)}>{row.country}</TableCell>
                                        <TableCell align="left"
                                            onClick={(e) => CellClickHandler(row._id, e)}>{row.specialization}</TableCell>
                                        <TableCell align="left">
                                            <WorkStatusContainer
